...
4. **Game** (`/game`) — 10 problems (or 9 for 구구단), 180-second hard cap **in challenge mode only**. Game ends on whichever comes first; remaining unanswered problems count as wrong. The session is **challenge** (timer + record save), **타임어택** (60s open-ended race + record save with `mode = timeAttack`), or **practice** (no timer, no record save). All three are opted into from the level-select segmented toggle; times-table runs are always practice. Flags travel via `Get.arguments['isPractice']` and `Get.arguments['isTimeAttack']` (the former auto-true when `tableNumber` is set, the latter ignored for times-table/mixed).
5. **Result** (`/result`) — shows correct/wrong counts, elapsed time, and persists a `GameRecord` (`finishedAt`, `type`, `level`, `correctCount`, `wrongCount`, `elapsedSeconds`, `mode`) via `RecordService`. Elapsed time is computed as `totalSeconds - secondsLeft.value` at finish, so it equals the full session length when the timer expires.
...
### Challenge vs Practice vs Time Attack

A run picks one of three shapes. **Challenge** (default) is the fixed 10-problem, 180s countdown that persists a `GameRecord`. **Practice** (`isPractice == true`) drops the timer and the record. **Time attack** (`isTimeAttack == true`) keeps the timer (60s countdown via `timeAttackSeconds`) but the problem list is open-ended: each submit appends one freshly generated problem and the timer is the only thing that can end the session. Time attack persists a record with `mode = SessionMode.timeAttack`; the Result screen's "신기록" comparison switches from min-elapsedSeconds (challenge) to max-correctCount within the same `(type, level, mode)` bucket.

Three modes are encoded as **two independent flags** rather than a single enum because times-table and mixed sessions already have their own toggles. The timer infrastructure is shared: a single `elapsed` Rx counts up every second; challenge/time-attack derive `remainingSeconds = totalSeconds - elapsed` (where `totalSeconds` is per-mode) and `_finish`-es at zero, while practice lets `elapsed` count up forever and never auto-finishes. `GameView` swaps the AppBar timer (countdown red ↔ informational gray elapsed), the title ("X / Y" ↔ "맞춤 N · 시도 M"), and the progress bar (time drain ↔ problem progression) based on the mode. Don't reintroduce the old `secondsLeft` countdown Rx — derive from `elapsed`.

Time attack is **excluded** from "perfect"-style rewards: it never produces a `_isPerfect` record (the in-flight problem is always unsolved by construction, and `_isPerfect` additionally requires `mode == challenge`), so master badges, "사칙연산 정복", and the perfect-games daily mission still measure challenge runs only. Cumulative counts (correct totals, combos, streak) include time-attack runs.
...
- **Dependencies of note**: `get` (navigation + state), `shared_preferences` (record storage + mute toggle), `audioplayers` (SFX), `lottie` (home/game animations), `flutter_launcher_icons` (dev). Typography is a bundled TTF — no `google_fonts` dependency (removed to keep the app fully offline; see below).
- **Typography**: app-wide font is **Jua** (주아), bundled as `assets/fonts/Jua-Regular.ttf` and declared in `pubspec.yaml`'s `fonts:` section. `lib/main.dart` sets `ThemeData(fontFamily: 'Jua', ...)` so every `TextStyle` inherits it. Don't hard-code `fontFamily` on individual `TextStyle`s — read from `Theme.of(context).textTheme.<style>.fontFamily` if you need to mix sizes (see `splash_view.dart`). To swap fonts globally, replace the TTF + change the single `fontFamily` string. No runtime network fetch; the font works offline from first launch (this is intentional for Play Store children's-app compliance — don't reintroduce `google_fonts`).
